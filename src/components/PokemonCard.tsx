...
const PokemonCardComponent: FC<PokemonCardProps> = ({ pokemon, index }) => {
	return (
		...
		>
			...
			<div className="w-[72px] h-[72px] shrink-0 flex items-center justify-center bg-[radial-gradient(circle,rgba(255,255,255,0.03)_0%,transparent_70%)] rounded-lg max-sm:w-14 max-sm:h-14">
				<img
					className="w-full h-full object-contain [image-rendering:pixelated] drop-shadow-[0_2px_4px_rgba(0,0,0,0.3)] transition-transform duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] group-hover:scale-115 group-hover:-rotate-3"
			...
			<div className="flex flex-col gap-0.5 min-w-0">
				...
				<span className="text-[0.7rem] font-semibold text-poke-muted tabular-nums">{pokemon.displayId}</span>
			</div>
		</li>
	);
};
